...
;(function($, window, document, undefined) {
	...
	Plugin.prototype.init = function() {
		...
		if (touchEventEnabled) {	// touch event를 지원하는 브라우저에서는 touch event에 리스너 등록
			$this.on('touchstart.cfTouchSwipe', {element: this.element, options: this.options}, onTouchStart);
			$this.on('touchmove.cfTouchSwipe', {element: this.element, options: this.options}, onTouchMove);
			$this.on('touchend.cfTouchSwipe', {element: this.element, options: this.options}, onTouchEnd);
		} else {		// 그렇지 않은 브라우저에서는 mouse event에 리스너 등록
			$this.on('mousedown.cfTouchSwipe', {element: this.element, options: this.options}, onTouchStart);
			$this.on('mousemove.cfTouchSwipe', {element: this.element, options: this.options}, onTouchMove);
			$this.on('mouseup.cfTouchSwipe', {element: this.element, options: this.options}, onTouchEnd);
		}
