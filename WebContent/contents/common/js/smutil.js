...
//									});
	...
	/**
	 * 카메라에서 생성된 파일 결과 리턴
	 * _sDirectory : 생성할 파일의 저장위치
	 * _sFileName : 생성할 파일명
	 * width : 파일의 width 값 (없으면 800px)
	 * height : 파일의 height 값 (없으면 600px)
	 * fileSize : 파일의 max size byte크기 (없으면 500kb)
	 */
	var callCamera = function(_sDirectory, _sFileName, callbackFunction, width, height, fileSize){
		
		// 확장자 명만 추출한 후 소문자로 변경
		var _fileLen = _sFileName.length;
		var _lastDot = _sFileName.lastIndexOf('.');
		var _fileExt = _sFileName.substring((_lastDot+1), _fileLen).toLowerCase();
		...
		if(!smutil.isEmpty(_sFileName) && !smutil.isEmpty(_sDirectory)){
//			var _nWidth = smutil.nullToValue(width, 800);
//			var _nHeight = smutil.nullToValue(height, 600);
			...
			LEMP.System.callCamera({
				...
				"_sDirectory" : _sDirectory,
				"_bAutoVerticalHorizontal" : true, // or false
				
				"_fCallback" : function(res) {
					
					if(!smutil.isEmpty(res.path)){
						var returnObj = {
								"result" : true, 
								"list" : [ { "origin_path" : res.path, "target_path" : res.path} ]
						}
	 ...
	 * fileFullPath : 원본 파일명에서 새로운 파일명으로 파일을 바꿀 이름
	 * callbackFunction : callbackfunction
	 * _sType : 갤러리에서 불러올 미디어 타입입니다. all, image, video가 있습니다.
	 * width : 파일의 width 값 (없으면 800px)
	 * height : 파일의 height 값 (없으면 600px)
	 * fileSize : 파일의 max size byte크기 (없으면 500kb)
	 */
	var callGallery = function(fileFullPath, callbackFunction, type, width, height, fileSize){
		...
		if(!smutil.isEmpty(fileFullPath)){
			var _sType = smutil.nullToValue(type, "image");
...
//			var _nHeight = smutil.nullToValue(height, 600);
			...
			LEMP.System.callGallery({
				"_sType" : "image",
				"_fCallback" : function(res) {
					
					if(!smutil.isEmpty(res.images) && !smutil.isEmpty(res.images[0].uri)){
						var _sSourcePath = res.images[0].uri;
						
						// 파일이 있는지 검사
						LEMP.File.exist({
							"_sSourcePath" : _sSourcePath ,
							"_fCallback"  : function(existRes){
