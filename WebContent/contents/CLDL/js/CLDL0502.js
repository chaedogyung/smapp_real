var page = {
	...
	init:function(arg)
	{
		page.cldl0502.inv_no = String(arg.data.param.inv_no);
		page.initInterface();
	}
	
	
	// 코드리스트를 불러올 공통코드 값을 저장하는 전역변수
	,cldl0502:{}
	...
	,initInterface : function()
	{
		...
		// 미집하 사유가 취급불가일경우 이미지 표시
		Handlebars.registerHelper('img_src', function(options){
			if (this.dlay_rsn_cd === "22") {
				return options.fn(this);
			}
		});
		...
		$(document).on("click",".btn.map",function(){
			// 개발미완료
			page.cldl0502.cldl_sct_cd="P";
			page.cldl0502.shcn_sct_cd=$(this).closest(".infoBox").attr("id");
		...
		// 이전페이지에서 넘겨받은 접수번호로 호출한 데이터를 출력
		page.invDtl();
	}
	...
	,invDtl:function(){
		...
		page.apiParam.param.baseUrl="smapis/cldl/invDtl";
		page.apiParam.param.callback="page.invDtlCallback";
		page.apiParam.data.parameters=page.cldl0502;
	...
	//상세화면 콜백
	, invDtlCallback : function(data){
		try{
			var res = data.data.list[0];
			if (data.data_count !== 0 && smutil.apiResValidChk(data)&& data.code=="0000") {
				// 상세정보 null check
				...
				res.bsc_fare=String(res.bsc_fare).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
				res.airf=String(res.airf).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
				res.pltg=String(res.pltg).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
				res.extch=String(res.extch).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
				
				
				
				//handlebars template 시작
				var template = Handlebars.compile($("#CLDL0502_list_template").html());
			
				$("#contents").html(template(res));
