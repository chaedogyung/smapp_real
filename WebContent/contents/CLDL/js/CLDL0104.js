var page = {
	...
	{
		var obj={
			"rsrv_mgr_no":String(arg.data.param.rsrv_mgr_no),
			"inv_no":String(arg.data.param.inv_no),
			"cldl_sct_cd":arg.data.param.cldl_sct_cd
		};
	...
	, invDtlCallback : function(data){
		try{
			...
			if (smutil.apiResValidChk(data) && data.code === "0000" && data.data_count !== 0 ) {
				...
				if (res.inv_no !== null) {
					res.dtlNo = res.inv_no.substr(0,4)+"-"+res.inv_no.substr(4,4)+"-"+res.inv_no.substr(8,4);
				}else {
					res.dtlNo = res.rsrv_mgr_no;
				}
				...
				// 송장번호가 있을때에만 지도버튼 표시
				Handlebars.registerHelper('map', function(options){
				...
				var template = Handlebars.compile($("#CLDL0104_list_template").html());
