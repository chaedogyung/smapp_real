var page = {
	...
	init:function(arg)
	{
		page.plag.rsrv_mgr_no = String(arg.data.param.rsrv_mgr_no);
		page.plag.inv_no = String(arg.data.param.inv_no);
		page.initInterface();
	}
	...
	,initInterface : function()
	{
		...
		$(document).on("click",".btn.map",function(){
			// 개발미완료
			data.cldl_sct_cd="P";
			data.inv_no=page.plag.inv_no;
			data.shcn_sct_cd=$(this).closest(".infoBox").attr("id");
	...
	, invDtlCallback : function(data){
		try{
			...
			if (data.data_count !== 0 && smutil.apiResValidChk(data)&& data.code=="0000") {
				...
				var template = Handlebars.compile($("#CLDL0305_list_template").html());
