...
var page = {
	...
	// 동의 상태 조회
	getMsgCnf: function() {
		// 임시로 토큰 저장(API 호출 후 삭제)
		LEMP.Properties.set({
			"_sKey" : "accessToken",
			"_vValue" : page.accessToken
		});
	...
	getMsgCnfCallback: function(res) {
		...
		if(res.code === "00" || res.code === "0000") {
			...
			if (data.lms_agg_yn === 'N' && smutil.isEmpty(data.dis_agg_sct_cd)) {	// 미입력 사용자
				// 통신사 설정
	...
	callbackBackButton : function() {
		LEMP.Window.alert({
			"_sTitle": "알림",
			"_vMessage": "자동 문자발송 미동의 또는 동의 항목을 선택 하시고 저장을 하셔야 합니다",
		});
	},
